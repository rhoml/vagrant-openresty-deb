...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |v|
  ...
  servers.each do |s, c|
    ...
    v.vm.define s do |b|
      ...
      b.vm.provision :shell, :inline => "echo \"export openresty_version=#{OPENRESTY_VERSION}\" >> /etc/profile.d/env_variables.sh"

      # Install necessary packages to compile Openresty and
